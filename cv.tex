...
\begin{itemize}
\item Vision-based Tissue Manipulation and Dissection using the da Vinci Research Kit (DVRK) and the cisst/SAW Software Environment, Computer Integrated Interventional Systems Laboratory, Johns Hopkins University, Visiting Student. (Jan. 2016 - April. 2017) \\
  Supervisor: \textit{Prof. Yunhui Liu, Prof. Peter Kazanzides, Prof. Russell H. Taylor, et al.}
  \begin{itemize}
  \item Help with the improvement of dvrk-ros and cisst-saw software  environment in terms of trajectory generator, MatLab wrapper, joint torque control interface, etc;
  ...
  \item Smart model-less deformable object manipulation in constrained environments using dVRK;
  \item Smart autonomous unknown deformable tissue/tool manipulation using the dVRK;
  \item Novel robust model-less framework for autonomous manipulation of unknown heterogeneous 3-D deformable Objects using dVRK;
  \item Soft tissue dissection strategy design by incorporating with vision-based soft tissue manipulation using dVRK;
  \item Dynamic robotic manipulation of soft objects using an uncalibrated camera using dVRK.
  \end{itemize}
